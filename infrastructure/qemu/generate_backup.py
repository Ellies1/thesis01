...
def start(config, machines):
    ...
    print(f"[DEBUG] config['infrastructure'] type: {type(config['infrastructure'])}")
    print(f"[DEBUG] config['infrastructure'] content: {config['infrastructure']}")


    try:
        raw_value = config["infrastructure"]["virtiofsd"]
    except Exception:
        raw_value = "false"

    virtiofsd_enabled = config["infrastructure"].get("virtiofsd", False)
    print(f"[DEBUG] virtiofsd raw value = {raw_value}, enabled = {virtiofsd_enabled}")
    using_kata = False
    if (
        "benchmark" in config
    ...
    for machine in machines:
        ...
        for ip, name in zip(
            ...
            print(f"DEBUG: virtiofsd_enabled={virtiofsd_enabled}, name={name}")
            if virtiofsd_enabled and name.startswith("cloud"):
                host_dir = f"/var/lib/libvirt/scaphandre/{name}"
                try:
                    os.makedirs(host_dir, exist_ok=True)
                    logging.info(f"Created Scaphandre directory: {host_dir}")
                except Exception as e:
                    logging.error(f"Failed to create directory {host_dir}: {str(e)}")
                    sys.exit(1)
